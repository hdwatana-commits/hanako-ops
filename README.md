...
## 目的
`商品を探す` 画面の上部に「今朝のおすすめ」を追加しました。登録済み商品をそのまま200件見せず、重複・再投稿リスク・投稿準備の不足・売れやすさ・運用優先度を計算し、最初に確認する商品をSランク最大30件へ絞ります。

## 使い方
1. 楽天ROOM/楽天市場URL、楽天検索、手入力で商品候補を登録します。
2. `商品を探す` を開き、`今朝のおすすめを作る` を押します。
3. Sランク商品だけ確認し、不要なものは `除外`、迷うものは `保留` にします。
4. 投稿する商品を選択し、`一括採用`、`画像生成キューへ`、`ROOM文生成へ` を使います。
5. 投稿パイプラインで `候補 → 採用 → 画像待ち → 文章完成 → 投稿済み` の流れを確認します。

## 追加ファイル
- `ops-engine.js`: canonicalProductId生成、商品名正規化、重複統合、再投稿可否、スコア、コレクション提案
- `supabase-ops-phase1.sql`: 将来の正規化DB用マイグレーション
- `docs/phase1-ops-redesign-plan.md`: 調査結果とPhase 1計画
- `sample-data/ops-phase1-products.json`: サンプル商品
- `tests/ops-engine-tests.html`: ブラウザで実行する最低限テスト

## KPI
アプリ内で以下をDailySelectionとして保存します。
- 収集候補数
- 保存候補数
- 初期表示件数
- 確認削減率
...
販売成果のクリック数・売上件数・売上金額・成果報酬は楽天ROOMから自動取得できないため、現段階では成果画面への手入力または今後のCSV取込で扱います。取得できない数値は推測保存しません。

## Supabase
既存同期は `hanako_app_data.payload` のJSONB保存を継続します。正規化テーブルを使う場合は、Supabase SQL Editorで `supabase-ops-phase1.sql` を実行してください。秘密情報やservice_roleキーはコードへ入れません。

## 既知の制約
- 自動収集の定期実行はPhase 2以降です。Phase 1は登録済み商品・楽天検索・URL取込を候補ソースにします。
- 画像類似度はPhase 3です。Phase 1は商品コード、URL、商品名、色、サイズで判定します。
- 類似投稿の左右比較専用画面はPhase 2です。Phase 1ではカード内の再投稿状態と理由で確認します。
- コレクションの自動登録は予約/提案までです。実際の楽天ROOM側への登録操作は手動です。

## テスト
`tests/ops-engine-tests.html` をブラウザで開くと、以下を確認できます。
- URL表記が異なる同一商品の統合
- 色違い判定
- SALE/新色/追加など販促語の正規化
- 60日以内の再投稿ブロック
...
- バッチ再実行時の冪等性
- API失敗時フォールバック
## Phase 2 OPS: 判断待ちと成果学習

- 商品画面に「判断待ち受信箱」と「コレクションルール」を追加しました。
- 高確信度の商品はコレクション登録予約へ進み、曖昧な商品だけ受信箱に入ります。
- 成果画面から楽天アフィリエイト等のCSVを取り込み、商品・投稿へ自動紐付けできます。
- ブランド、カテゴリ、価格帯、コレクション、スコア帯ごとの成果を集計します。
...
- ROOM投稿画面に「新規コレクションを作る」を追加しました。
- 選択中の商品を中心に、コレクション件名、本文、コレクション表紙画像プロンプト、AIへ渡すまとめプロンプトを一括生成できます。
- 方向性は「商品からAIおすすめ」「旅行」「通勤」「デート」「骨格ウェーブ」「冷房対策」「3000円以下」「同ブランドまとめ」から選べます。
- 各項目は個別コピー、または全部まとめてコピーできます。

## Phase 4 OPS: AI自己学習エンジン

- ホームに「AIが毎日学習する」ダッシュボードを追加しました。
- 投稿、AI予測、クリック、売上、報酬、ブランド、カテゴリ、価格帯、背景、コピー、画像特徴を学習イベントとして保存します。
- 毎日1回、AI反省会、改善案、重み変更提案、勝ちパターン、負けパターン、探索枠、改善シミュレーションを作成します。
- Knowledge DBを使って「今日は何が売れる？」「売れない理由は？」「今週おすすめブランドは？」に根拠付きで答えます。
- 探索枠は10〜20%を目安にし、実績重視と未知商品の検証を両立します。
- SupabaseへPhase 4テーブルを追加する場合は `supabase-ops-phase4.sql` をSQL Editorで実行してください。
