...
async function drawHanakoTeacherPanel(ctx, coordinate, analysis) {
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(bubbleX + bubbleWidth, bubbleY + 76);
  ctx.lineTo(bubbleX + bubbleWidth + 28, bubbleY + 96);
  ctx.lineTo(bubbleX + bubbleWidth, bubbleY + 112);
  ctx.closePath();
  ctx.fillStyle = "#fff";
  ctx.fill();
  ctx.strokeStyle = "#e4b9c8";
...
・完成コーデを画面の約70%で大きく見せ、ハナコ先生は右下の安全な余白へ約22〜25%の大きさで、顔と表情がはっきり分かるよう配置する
・ハナコ先生を左端へ置かない。右端から約6〜8%の余白を残し、頭、髪、リボン、丸いアイコンの輪郭を一切切らない
...
・右下の白い部分には、服、小物、かざり、影を置かない。ただし「ハナコ先生の吹き出し解説」では、選んだ先生アイコンだけを右端の安全域へ置いてよい
...
・右下の背景が自然な白になっている。「ハナコ先生の吹き出し解説」では、その白い安全域に先生アイコンが大きく表示されている
...
function getCoordinateImagePatternInstruction(pattern, coordinate) {
  const patterns = {
    ...
    "ハナコ先生の吹き出し解説": "完成コーデを大きく見せ、今回選ばれたハナコの丸い先生アイコンを右下の安全な余白へ、顔がはっきり見える約22〜25%の大きさで配置する。先生は右端から6〜8%離して輪郭を切らず、吹き出しは先生の左側へ置く。先生の吹き出し1個で悩みの解決を短く総評し、主役商品と小物の近くには別途、手書き風の矢印・丸囲み・下線・短いメモを3〜5個入れる。上部10%は短い見出し、中央約70%は完成コーデ、下部約20%は先生解説として毎回同じ情報階層を保つ。ファッション誌の解説ページのように、かわいく高品質で読みやすく整理する。",
