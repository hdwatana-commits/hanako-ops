...
・都市情報だけで独立した段落にしない。ファッションの課題解決の理由として、1〜3行で読みやすく差し込む
...
・本文は全体で${captionLength}。短すぎる感想文、長すぎる商品説明、箇条書きの羅列にはしない
...
・「${greeting}」の次の本文冒頭100文字くらいで必ず読者をフックする
・冒頭100文字は、1. よくある悩みへの共感、2. このコーデの結論、3. 明日まねできる学び、の3つを短く入れる
・冒頭100文字の中に、主役商品、解決できる悩み「${coordinate.concern}」、かわいく見える理由を必ず入れる
・冒頭100文字前後で一度空行を入れ、そこで「今日はこれを読めば何が分かるか」が伝わっている状態にする
・冒頭を広告っぽくしない。「買って」「急いで」「神」だけで釣らず、読者が自分の服選びに使える気づきで引き込む
・その後は「なぜ解決するか → 具体的な着こなしの見方 → 都市や場所の空気感 → 買う前チェック → 保存したくなる締め」の順へ自然につなげる
・全体は読みやすい4段落にする。1段落は2〜3文まで。スマホで読んでも詰まって見えないように空行を入れる
