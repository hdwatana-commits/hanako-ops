...
async function imageFor(platform: string, look: Record<string, unknown>, index: number, personalReference: string, syntheticReference: string) {
  ...
  const prompt = `IMPORTANT OUTPUT CONTRACT: Generate exactly ONE image file for this API call (n=1). The canvas must show ONE continuous scene and ONE portrait shot. Never compose multiple camera frames, side-by-side portraits, a diptych, triptych, collage, contact sheet, grid, storyboard or comic panels inside this image. Other shots will be made by separate API calls. Create one photorealistic vertical 4:5 editorial photo of the same adult woman in a three-photo series. The first supplied image is the person's identity reference: retain recognizable facial features and adult appearance. ${syntheticReference ? "The second supplied image is the prior AI-generated shot: use it only for continuity of the outfit, hair and setting; do not copy its borders or create a before-and-after composition." : "Establish a consistent outfit, hair and setting for the series."} Scene: ${look.scene}. Same exact location in all three: ${look.location}. Same exact outfit and hair in all three: ${look.outfit}, ${look.hair}. Light: ${look.light}. This API call makes shot ${index + 1} only. Pose: ${pose}. Expression: ${expression}. Framing: ${composition}. ${instagram ? "Charming, elegant, subtly flirtatious fashion portrait." : "Warm, approachable, cute and conversation-inviting portrait."} Fully opaque clothing, no underwear visible. Indoors no shoes. Natural anatomy, realistic hands and eyes, believable fabric, authentic camera light. No artificial skin, distortions, extra limbs or fingers. No text, logos, watermarks, split panels or borders. Return just the single unpartitioned image.`;
